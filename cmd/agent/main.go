...
var (
	...
	advertiseEndpFlag    = flag.String("advertise-endpoint", "", "pin the public host:port other peers dial for direct WireGuard (e.g. a VPS IP or DNS name with the published listen port); skips STUN discovery of it and survives NAT re-checks — for hosts whose reachable address observation gets wrong (docker hairpin, static port-forwards)")
